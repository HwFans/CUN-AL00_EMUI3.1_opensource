...
/*Bill add in 20150917 , for 99% charging post time*/
...
#define HQ_GET_BAT_TEMP _IOW('k',13,int)   //liuxiong add
/* add for meta tool----------------------------------------- */
#define Get_META_BAT_VOL _IOW('k', 10, int)
#define Get_META_BAT_SOC _IOW('k', 11, int)
/* add for meta tool----------------------------------------- */

#define GET_BATTERY_MESSAGE _IOW('k', 12, int)
...
//allen wy 201506007 add
extern kal_bool gFG_Is_Charging;

extern struct timespec g_sleep_total_time; //[HQ00055994],wangjianhui, al806 mtk_code合入P26_2
/* ////////////////////////////////////////////////////////////////////////////// */
/* FOR ADB CMD */
/* ////////////////////////////////////////////////////////////////////////////// */
/* Dual battery */
...
struct battery_data {
	...
	int present_smb;
	int adjust_power;
	/*huaqin zam add for huawei 20150811*/
	int capacity_fcc;
	int capacity_rm;
};
...
/* ///////////////////////////////////////////////////////////////////////////////////////// */
//lupingzhong add for boot with vibrator
#ifdef CONFIG_HQ_BOOT_WITH_VIBRATOR
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		/*Bill add for runin battery test start, study from lx,  20150815*/
		if(g_runin_test_flag)
		{
		        battery_log(BAT_LOG_CRTI, "[BATTERY] battery_get_property Battery SOC:%d!!\n\r",BMT_status.SOC);  
...
{
	//add by zym
/*	if(g_boot_mode_ex == 11)
		bat_data->BAT_CAPACITY = 43;
	else*/
	...
	/*huaqin zam add for huawei 20150811*/
	bat_data->capacity_fcc=gFG_BATT_CAPACITY_aging;
...
#ifdef POST_99TO100_CHARGING_TIME
/*Bill add in 20150917, for 99% , charging time counter*/
	...
	if (BMT_status.bat_full == KAL_TRUE)	/* charging full first, UI tracking to 100% */
	{
		...
		/*Bill add for huawei's test : charging is not full , but the charging time over 99% > 15min , force the percentage
			to 100% 
		*/
...
/*Bill add in 20150916 , for sync at 50%*/
extern kal_int32 gFG_50_vlot;
extern kal_int32 g_tracking_point_50;

extern kal_int32 gFG_14_vlot;
extern kal_int32 g_tracking_point_14;
#define TRACKING_TIME 60
/*Bill add end*/
static kal_bool mt_battery_nPercent_tracking_check(void)
{
	kal_bool resetBatteryMeter = KAL_FALSE;
	kal_int32 tracking_time = 60; /*14%, 41% 60s sync*/
#if defined(SOC_BY_HW_FG)
	static kal_uint32 timer_counter = (NPERCENT_TRACKING_TIME / BAT_TASK_PERIOD);
	static kal_uint32 timer_counter2 = (TRACKING_TIME / BAT_TASK_PERIOD);
	static kal_uint32 timer_counter3 = (NPERCENT_TRACKING_TIME / BAT_TASK_PERIOD);

	if (BMT_status.nPrecent_UI_SOC_check_point == 0)
		return KAL_FALSE;
			battery_log(BAT_LOG_CRTI,"allen wy  BMT_status.ZCV = %d--  BMT_status.UI_SOC = %d --  gFG_14_vlot = %d --  g_tracking_point_14 = %d  gFG_50_vlot = %d   g_tracking_point_50 = %d    BMT_status.nPercent_ZCV = %d BMT_status.nPrecent_UI_SOC_check_point = %d \n  ", 
						    BMT_status.ZCV,BMT_status.UI_SOC,gFG_14_vlot,g_tracking_point_14,gFG_50_vlot,g_tracking_point_50,BMT_status.nPercent_ZCV,BMT_status.nPrecent_UI_SOC_check_point);

#if 1
	/*Bill add in 20151030 , for sync soc at 14% , for huawei proj*/
	/* fuel gauge ZCV < 14%, but UI > 14%,  14% can be customized */
	if ((BMT_status.ZCV <= gFG_14_vlot)
	    && (BMT_status.UI_SOC > g_tracking_point_14)) {
		if (timer_counter3 == (NPERCENT_TRACKING_TIME / BAT_TASK_PERIOD))	/* every x sec decrease UI percentage */
		{
			BMT_status.UI_SOC--;
			timer_counter3 = 1;
		} else {
		...
		battery_log(BAT_LOG_CRTI,
				    ...
				    BMT_status.ZCV, gFG_14_vlot, BMT_status.UI_SOC,
				    g_tracking_point_14);
	} else if ((BMT_status.ZCV > gFG_14_vlot)
		   && (BMT_status.UI_SOC == g_tracking_point_14)) {
		/* UI less than 15 , but fuel gague is more than 15, hold UI 15% */
		timer_counter3 = (NPERCENT_TRACKING_TIME / BAT_TASK_PERIOD);
		resetBatteryMeter = KAL_TRUE;

		battery_log(BAT_LOG_CRTI,
				    "[nPercent] ZCV %d > BMT_status.14Percent_ZCV %d and UI SOC=%d, then keep %d.\n",
				    BMT_status.ZCV, gFG_14_vlot, BMT_status.UI_SOC,
				    g_tracking_point_14);
	} else {
		timer_counter3 = (NPERCENT_TRACKING_TIME / BAT_TASK_PERIOD);
	...
	/*Bill add in 20150916 , for sync soc at 50% , for huawei proj*/
	/* fuel gauge ZCV < 50%, but UI > 50%,  50% can be customized */
#endif
	/*Bill add end*/

#if 1
	/* fuel gauge ZCV < 2%, but UI > 2%,  2% can be customized */
	if ((BMT_status.ZCV <= BMT_status.nPercent_ZCV)
		...
		battery_log(BAT_LOG_CRTI,
		   ...
		   && (BMT_status.UI_SOC == BMT_status.nPrecent_UI_SOC_check_point)) {
		/* UI less than 2 , but fuel gague is more than 2, hold UI 2% */
...
static void battery_update(struct battery_data *bat_data)
{
    ...
    if( (BMT_status.charger_exist == KAL_TRUE) && (BMT_status.bat_charging_state != CHR_ERROR) && !g_charger_should_not_open)
 {
		if (BMT_status.bat_exist) {	/* charging */
			if (BMT_status.bat_vol <= V_CHARGINT_0PERCENT_TRACKING) {
		...
		battery_log(BAT_LOG_CRTI,"[battery_update]zcy-bat_data->BAT_STATUS = POWER_SUPPLY_STATUS_NOT_CHARGING\n");
...
/*Bill add for ui soc in 20150917*/
#if !defined(CUST_CAPACITY_OCV2CV_TRANSFORM)
	if (BMT_status.charger_exist == KAL_FALSE) {
		if (BMT_status.UI_SOC > previous_UI_SOC && previous_UI_SOC >= 0)
			BMT_status.UI_SOC = previous_UI_SOC;
	}
#endif
	previous_UI_SOC = BMT_status.UI_SOC;
	...
	if (BMT_status.UI_SOC <= 1) {
	    ...
	    /*Bill add  20150815 for runin battery test start , study from lx*/
            if(g_runin_test_flag) {
                battery_log(BAT_LOG_CRTI, "[BATTERY] set_rtc_spare_fg_value and Set Battery SOC:%d!!\n\r",BMT_status.SOC);  
...
void mt_battery_GetBatteryData(void)
{
	...
	static kal_int32 bat_sum, icharging_sum, temperature_sum,current_sum;
	...
	battery_log(BAT_LOG_CRTI,"huaqin bat_vol=%d \n",bat_vol);
	...
	if (bat_meter_timeout == KAL_TRUE || bat_spm_timeout == TRUE || fg_wake_up_bat== KAL_TRUE) 
	{
		...
		//if (bat_spm_timeout == true)
			...
			/*Bill removed 20150918 ,  invalid , we have sync method between ui_soc and soc in late phase */
			/*BMT_status.UI_SOC = BMT_status.SOC;*/
			/*Bill removed end*/
...
#endif
		#if !defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
		if (BMT_status.charger_vol >= V_CHARGER_HIGHT_WARNING) {
		#else
		if (BMT_status.charger_vol >= v_charger_max) {
		#endif
			battery_log(BAT_LOG_CRTI, "zjy1 [BATTERY]%d > %d Charger over voltage !!\r\n", BMT_status.charger_vol,V_CHARGER_HIGHT_WARNING);
			BMT_status.charger_protect_status = charger_OVER_VOL;
			BMT_status.bat_charging_state = CHR_ERROR;
			status = PMU_STATUS_FAIL;
		} else if ((BMT_status.charger_vol > 5750) && (BMT_status.charger_protect_status == charger_OVER_VOL)) {
			battery_log(BAT_LOG_CRTI, "zjy1 [%s]hold not charge", __func__);
			status = PMU_STATUS_FAIL;
		} else 
...
#define JEITA_TEMP_POS_45_TO_POS_60_BAT_REPORT  (4100)
static void mt_battery_notify_VBat_check(void)
{
        if ((BMT_status.bat_vol >= JEITA_TEMP_POS_45_TO_POS_60_BAT_REPORT) && 
			(TEMP_POS_45_THRESHOLD < BMT_status.temperature))
        {
                g_BatteryNotifyCode |= 0x0080;
                //battery_log(BAT_LOG_CRTI, "[BATTERY] bat_vlot(%ld) > 4350mV JEITA_TEMP_POS_45_TO_POS_60_CC2TOPOFF_THRESHOLD\n",BMT_status.bat_vol);
        }
...
#if defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
	else if ((BMT_status.temperature < TEMP_NEG_10_THRESHOLD)&& (BMT_status.charger_exist == KAL_TRUE)) {
...
static void mt_battery_notify_current_check(void)
{
	static kal_uint32 last_soc = 0;
	kal_uint32 current_soc = 0;
	...
	/*Bill remove CC_mode case , if 100%->89% , still warning , in 20150925*/
	if((BMT_status.charger_exist == KAL_TRUE))
	{
	/*Bill add start in 20150910,  over power consume warning , < (25-10) 15%, low power warning , not this way*/	
		if (BMT_status.SOC > 25 ||( last_soc > 25 && BMT_status.SOC > 15))
		{
			current_soc = BMT_status.SOC;
			...
			{
				if ((last_soc - current_soc) > 10)
				{
				/*Bill add in 20150925, for full charging state: if soc:100%->89% , force exiting battery_full state, 
				  avoid soc reducing all the time , however , ui_soc still keep 100%!
				*/
				...
				/*Bill and end 20150925*/
					last_soc = current_soc;				
					g_BatteryNotifyCode |= 0x0040;
					battery_log(BAT_LOG_CRTI, "[BATTERY] BATTERY_NOTIFY_CASE_0006_CURRENT (%x)\n",
						g_BatteryNotifyCode);
				}
			}					
		}
...
void mt_battery_notify_check(void)
{
	...
	if (g_BN_TestMode == 0x0000) {	/* for normal case */
		...
		mt_battery_notify_current_check();
	} else {		/* for UI test */
...
#if defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
		/* ignore default rule */
	/*The device should power off when temprature is lower than -20'C , for huawei's request*/
	if (BMT_status.temperature <  (-20) ) {

		struct battery_data *bat_data = &battery_main;
		struct power_supply *bat_psy = &bat_data->psy;

		battery_log(BAT_LOG_CRTI,
				    "[Battery] Tbat(%d) < -20'C, system need power down.\n",
				    BMT_status.temperature);
...
extern kal_int32 fgauge_read_v_by_capacity(int bat_capacity);
kal_uint32 set_bat_charging_current_limit(int current_limit);
unsigned int flag_call = 1;

extern kal_int32 gFG_capacity_by_c;//1cee636... [HQ00055994],wangjianhui, al806 mtk_code合入P26_2
void BAT_thread(void)
{
	...
	battery_log(BAT_LOG_CRTI, 
						"[%s] ZCV %d BMT_status.nPercent_ZCV %d and UI SOC=%d, then keep %d. %d\n",
						__func__, BMT_status.ZCV, BMT_status.nPercent_ZCV, BMT_status.UI_SOC,
						BMT_status.nPrecent_UI_SOC_check_point, three_percent_zcv);
...
//lupingzhong add for vibration when press powerkey until lighting backlight in usb mode start
    #ifdef CONFIG_HQ_BOOT_WITH_VIBRATOR
        ...
        if(kernel_vibrate_flag == false){//vibration only once
			if(timer_end -timer_init >= PWRKEY_LONG_PRESS_TIME){
				...
				vibr_Disable_HW();
				mdelay(200);
				printk("[kernel:battery_common] Enable_Vibrator--->>>Disable_Vibrator!!!\r\n");
				timer_init = 0;
				timer_end = 0;
			}
		}
		#endif
	#endif 
    //lupingzhong add for vibration end.
	if (BMT_status.charger_exist == KAL_TRUE) {
	...
	/*Bill add in 20151008 , for pmic charging register dump*/
...
static long adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ADC_CHANNEL_READ:
		...
		break;
	case GET_BATTERY_MESSAGE:
		{
			int battery_msg[4]={0};
			user_data_addr = (int *)arg;
			battery_msg[0] = (battery_meter_get_battery_current()) / 10;
			battery_msg[1] = BMT_status.ICharging;
			battery_msg[2] = battery_meter_get_charging_current();
			battery_msg[3] = BMT_status.bat_vol;

			ret = copy_to_user(user_data_addr, battery_msg, 16);
		}
	    	break;
	...
	case Get_META_BAT_VOL:
		...
		adc_out_data[1] = (temp_buf[0]<<8 | temp_buf[4]);
        ...
        case HQ_GET_BAT_TEMP://liuxiong add 20131122 end
                user_data_addr = (int *)arg;
                ret = copy_from_user(adc_in_data, user_data_addr, 8);
	...
	case Get_META_BAT_SOC:
		...
		case HQ_Get_BAT_VOL:
			...
			battery_log(BAT_LOG_CRTI,"get_bat_vol %d \n", adc_out_data[0]);
#ifdef CONFIG_HQ_AL813_PROJECT
                        adc_out_data[0] += 18;
#else
                        adc_out_data[0] += 10;
#endif
...
static int charging_current_read(struct seq_file *m, void *v)
{
	if( upmu_is_chr_det() == KAL_TRUE )
	{
		seq_printf(m,
		   "AvgICharging=%d,\nICharging=%d\n",
		   BMT_status.ICharging, battery_meter_get_charging_current());
	}
	else
	{
		seq_printf(m,
		   "charger is not exist,please plug in\n");
	}
	return 0;
}
static int proc_open_charging_current(struct inode *inode, struct file *file)
{
	return single_open(file, charging_current_read, NULL);
}
static const struct file_operations charging_current_cmd_proc_fops = {
	.open = proc_open_charging_current,
	.read = seq_read,
};
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
extern char* fgauge_get_battery_manufacturer(void);
static int battery_manufacturer_read(struct seq_file *m, void *v)
{
	seq_printf(m,"battery manufacturer: %s\n",fgauge_get_battery_manufacturer());
	return 0;
}
static int proc_open_battery_manufacturer(struct inode *inode, struct file *file)
{
	return single_open(file, battery_manufacturer_read, NULL);
}
static const struct file_operations battery_manufacturer_cmd_proc_fops = {
	.open = proc_open_battery_manufacturer,
	.read = seq_read,
};
#endif
static int mt_batteryNotify_probe(struct platform_device *dev)
{
	...
	if (!battery_dir) {
		...
		proc_create("charging_current_cmd", S_IRUGO , battery_dir, &charging_current_cmd_proc_fops);
		battery_log(BAT_LOG_CRTI, "charging_current_cmd\n");
		#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
		proc_create("battery_manufacturer", S_IRUGO , battery_dir, &battery_manufacturer_cmd_proc_fops);
		battery_log(BAT_LOG_CRTI, "battery_manufacturer\n");
		#endif
