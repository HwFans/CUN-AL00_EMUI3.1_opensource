...
kal_int32 g_Q_MAX_POS_50[] = {Q_MAX_POS_50,Q_MAX_POS_f_50};
kal_int32 g_Q_MAX_POS_25[] = {Q_MAX_POS_25, Q_MAX_POS_f_25};
kal_int32 g_Q_MAX_POS_0[] = {Q_MAX_POS_0, Q_MAX_POS_f_0};
kal_int32 g_Q_MAX_NEG_10[] = {Q_MAX_NEG_10, Q_MAX_NEG_f_10};
kal_int32 g_Q_MAX_POS_50_H_CURRENT[] = {Q_MAX_POS_50_H_CURRENT, Q_MAX_POS_f_50_H_CURRENT};
kal_int32 g_Q_MAX_POS_25_H_CURRENT[] = {Q_MAX_POS_25_H_CURRENT, Q_MAX_POS_f_25_H_CURRENT};
kal_int32 g_Q_MAX_POS_0_H_CURRENT[] = {Q_MAX_POS_0_H_CURRENT, Q_MAX_POS_f_0_H_CURRENT};
kal_int32 g_Q_MAX_NEG_10_H_CURRENT[] = {Q_MAX_NEG_10_H_CURRENT, Q_MAX_NEG_f_10_H_CURRENT};
/* ///////////////////////////////////////////////////////////////////////////////////////// */
...
kal_int32 gFG_3_vlot = 3590;
/*Bill add in 20150916 , for sync at 50%*/
kal_int32 gFG_50_vlot = 3809;
kal_int32 g_tracking_point_50 = 41;
/*Bill add end*/

/*Bill add in 20151030*/
kal_int32 gFG_14_vlot = 3720;
kal_int32 g_tracking_point_14 = 14;
/*Bill add end*/
...
#define SELF_DISCHARGE_CHECK_THRESHOLD 5   //modify by Bill  10->5
#endif
...
#define BATTERY_SUNNYWODA 0
#define BATTERY_POLYMER   1
kal_int32 g_battery_id_voltage[2] = {100000,1100000};
...
void fgauge_get_profile_id(void)
{
	...
	if (ret != 0)
		bm_print(BM_LOG_CRTI, "zjy [fgauge_get_profile_id]id_volt read fail\n");
	else
		bm_print(BM_LOG_CRTI, "zjy [fgauge_get_profile_id]id_volt = %d\n", id_volt);
...
/*
	...
	if ((1100000 > id_volt)&&(id_volt > 900000))
		g_fg_battery_id = BATTERY_SUNNYWODA;
	else if ((200000 > id_volt)&&(id_volt > 100000))
		g_fg_battery_id = BATTERY_POLYMER;
	else 
		{
			battery_not_match = 1;
...
{
	return sizeof(battery_profile_temperature[g_fg_battery_id]) / sizeof(BATTERY_PROFILE_STRUC);
}

int fgauge_get_saddles_r_table(void)
{
	return sizeof(r_profile_temperature[g_fg_battery_id]) / sizeof(R_PROFILE_STRUC);
}
...
kal_int32 fgauge_read_v_by_capacity(int bat_capacity)
{
	...
	if (bat_capacity < (profile_p + 0)->percentage) {
		printk("[fgauge_read_v_by_capacity] fgauge get ZCV profile low %d : %d keep 3.7\n", bat_capacity, (profile_p + 0)->percentage);
		return 3700;
	}
	if (bat_capacity > (profile_p + saddles - 1)->percentage) {
		printk("[fgauge_read_v_by_capacity] fgauge get ZCV profile high %d : %d saddless: %d keep 3.7\n", bat_capacity, (profile_p + saddles - 1)->percentage, saddles);
		return 3700;
	}
...
EXPORT_SYMBOL_GPL(fgauge_read_v_by_capacity);
...
/*Bill add start in 20151124*/
extern char* saved_command_line;
int get_Ubvbat(void)
{
  char *ptr;
  int ubvbat1;
  ptr=strstr(saved_command_line,"Ubvbat=");
  if (ptr == NULL)
  {
	return 0;
  }
...
/*Bill add start in 20151123 , for charging boot , if hwocv is not correct , using the ubvbat as init ocv*/
	int ubvbat=0;
	kal_int32 gFG_capacity_by_ubvbat = 0;
	ubvbat=get_Ubvbat();
	bm_print(BM_LOG_CRTI, "[FGADC] TEST ubvbat=%d\n", ubvbat);
	if (ubvbat != 0)
	{
		if (g_boot_reason == BR_USB && bat_is_charger_exist() == KAL_TRUE)
		{
			ubvbat = (ubvbat + 45);
			gFG_capacity_by_ubvbat = fgauge_read_capacity_by_v(ubvbat);
			if ( abs(ubvbat - gFG_voltage) > 50 )
			{
				gFG_voltage = ubvbat;
				gFG_capacity_by_v = gFG_capacity_by_ubvbat;
				bm_print(BM_LOG_CRTI, "|ubvbat - hwocv| > 50mv , use ubvbat : %d " , ubvbat);
			}
		}
	}
/*Bill add end*/
	...
	sprintf(doddbg,"rtc:%d hwocv:(%d:%d) swocv:(%d,%d) ubvbat:(%d,%d) battery_remove:%d %d %d %d",g_rtc_fg_soc,gFG_voltage,gFG_capacity_by_v,gFG_voltage_init,gFG_capacity_by_v_init,ubvbat, gFG_capacity_by_ubvbat, is_battery_remove_pmic(),
...
*/
		{
			...
			{
				/*Bill modify in 20150925 , force use hw_ocv as init soc ; sw_soc is not accurate*/
				#if 0 
	...
	/*track 3%*/
	gFG_3_vlot = fgauge_read_v_by_capacity((100 - g_tracking_point));
	//gFG_3_vlot = (gFG_3_vlot);  /*down 5mv ,  */
	bm_print(BM_LOG_CRTI, "[FGADC] gFG_3_vlot = %dmV\n", gFG_3_vlot);
	if ((gFG_3_vlot > 3750) || (gFG_3_vlot < 3450)) {
		bm_print(BM_LOG_CRTI, "[FGADC] gFG_3_vlot(%d) over range, reset to 3659\n",
			 gFG_3_vlot);
		gFG_3_vlot = 3640;
	}

	/*Bill add in 20150916 , for sync soc at 41%*/
#if 1
	gFG_14_vlot = fgauge_read_v_by_capacity((100 - g_tracking_point_14));
	//gFG_14_vlot = (gFG_14_vlot); /*down 10mv*/
	bm_print(BM_LOG_CRTI, "[FGADC] gFG_14_vlot = %dmV\n", gFG_14_vlot);
	if ((gFG_14_vlot > 3800) || (gFG_14_vlot < 3600))
	{
		bm_print(BM_LOG_CRTI, "[FGADC] gFG_14_vlot(%d) over range, reset to 3720\n",
			 gFG_14_vlot);
		gFG_14_vlot = 3720;
	}

	gFG_50_vlot = fgauge_read_v_by_capacity((100 - g_tracking_point_50));
	bm_print(BM_LOG_CRTI, "[FGADC] gFG_50_vlot = %dmV\n", gFG_50_vlot);
	if ((gFG_50_vlot > 3900) || (gFG_50_vlot < 3700))
	{
		bm_print(BM_LOG_CRTI, "[FGADC] gFG_50_vlot(%d) over range, reset to 3840\n",
			 gFG_50_vlot);
		gFG_50_vlot = 3809;
	}
#endif
	...
	if ((gFG_15_vlot > 3750) || (gFG_15_vlot < 3450)) {
		bm_print(BM_LOG_CRTI, "[FGADC] gFG_15_vlot(%d) over range, reset to 3659\n",
			 gFG_15_vlot);
		gFG_15_vlot = 3659;
	}
...
kal_int32 battery_meter_get_battery_nPercent_zcv(void)
{
	...
	return 3700;
#else
	return gFG_3_vlot;	/* 15% zcv,  15% can be customized by 100-g_tracking_point */
#endif
}
...
static void battery_aging_check(void)
{
	...
	if (hw_ocv_after_sleep < vbat) {
		...
		if (DOD_hwocv > DOD_now
			...
			gFG_DOD1 = gFG_DOD0;
			reset_parameter_car();
			BMT_status.UI_SOC = (100-DOD_hwocv);
			bm_print(BM_LOG_CRTI,
				 "[self-discharge check] reset to HWOCV. dod_ocv(%d) dod_now(%d) ui_soc(%d)\n",
				 DOD_hwocv, DOD_now, BMT_status.UI_SOC);
		}
