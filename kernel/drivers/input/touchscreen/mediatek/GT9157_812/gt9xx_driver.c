...
static const struct i2c_device_id tpd_i2c_id[] = {{"gt9157", 0}, {}};
static unsigned short force[] = {0, 0xBA, I2C_CLIENT_END, I2C_CLIENT_END};
static const unsigned short *const forces[] = { force, NULL };
//static struct i2c_client_address_data addr_data = { .forces = forces,};
static struct i2c_board_info __initdata i2c_tpd = { I2C_BOARD_INFO("gt9157", (0xBA >> 1))};
...
#if GTP_GESTURE_WAKEUP  // liguangxian  20140917
...
#define GT91XXGAA_CONFIG_PROC_FILE     "gesture_enableaa"
...
#define GT91XXGBB_CONFIG_PROC_FILE     "gesture_enablebb"
...
#define GT91XXGAB_CONFIG_PROC_FILE     "gesture_enableab"
...
#define GT91XXGBA_CONFIG_PROC_FILE     "gesture_enableba"
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	{
		GTP_INFO("create proc entry %s success", GT91XXGE_CONFIG_PROC_FILE);
	}
	...
	{
		GTP_INFO("create proc entry %s success", GT91XXGC_CONFIG_PROC_FILE);
	}
		gt91xxgv_config_proc = proc_create(GT91XXGV_CONFIG_PROC_FILE, 0666, NULL, &configgv_proc_ops);
	...
	{
		GTP_INFO("create proc entry %s success", GT91XXGV_CONFIG_PROC_FILE);
	}
	...
	{
		GTP_INFO("create proc entry %s success", GT91XXGM_CONFIG_PROC_FILE);
	}
		gt91xxgo_config_proc = proc_create(GT91XXGO_CONFIG_PROC_FILE, 0666, NULL, &configgo_proc_ops);
	...
	{
		GTP_INFO("create proc entry %s success", GT91XXGO_CONFIG_PROC_FILE);
	}
	...
	{
		GTP_INFO("create proc entry %s success", GT91XXGW_CONFIG_PROC_FILE);
	}
		gt91xxgs_config_proc = proc_create(GT91XXGS_CONFIG_PROC_FILE, 0666, NULL, &configgs_proc_ops);
	...
	{
		GTP_INFO("create proc entry %s success", GT91XXGS_CONFIG_PROC_FILE);
	}
		gt91xxgz_config_proc = proc_create(GT91XXGZ_CONFIG_PROC_FILE, 0666, NULL, &configgz_proc_ops);
	...
	{
		GTP_INFO("create proc entry %s success", GT91XXGZ_CONFIG_PROC_FILE);
	}
		gt91xxgaa_config_proc = proc_create(GT91XXGAA_CONFIG_PROC_FILE, 0666, NULL, &configgaa_proc_ops);
	if (gt91xxgaa_config_proc == NULL)
	{
		GTP_ERROR("create_proc_entry %s failed\n", GT91XXGAA_CONFIG_PROC_FILE);
	}
	else
	{
		GTP_INFO("create proc entry %s success", GT91XXGAA_CONFIG_PROC_FILE);
	}
		gt91xxgbb_config_proc = proc_create(GT91XXGBB_CONFIG_PROC_FILE, 0666, NULL, &configgbb_proc_ops);
	if (gt91xxgbb_config_proc == NULL)
	{
		GTP_ERROR("create_proc_entry %s failed\n", GT91XXGBB_CONFIG_PROC_FILE);
	}
	else
	{
		GTP_INFO("create proc entry %s success", GT91XXGBB_CONFIG_PROC_FILE);
	}
		gt91xxgab_config_proc = proc_create(GT91XXGAB_CONFIG_PROC_FILE, 0666, NULL, &configgab_proc_ops);
	if (gt91xxgab_config_proc == NULL)
	{
		GTP_ERROR("create_proc_entry %s failed\n", GT91XXGAB_CONFIG_PROC_FILE);
	}
	else
	{
		GTP_INFO("create proc entry %s success", GT91XXGAB_CONFIG_PROC_FILE);
	}
		gt91xxgba_config_proc = proc_create(GT91XXGBA_CONFIG_PROC_FILE, 0666, NULL, &configgba_proc_ops);
	if (gt91xxgba_config_proc == NULL)
	{
		GTP_ERROR("create_proc_entry %s failed\n", GT91XXGBA_CONFIG_PROC_FILE);
	}
	else
	{
		GTP_INFO("create proc entry %s success", GT91XXGBA_CONFIG_PROC_FILE);
	}
	...
	{
		GTP_INFO("create proc entry %s success", GT91XXGCC_CONFIG_PROC_FILE);
	}
...
static int touch_event_handler(void *unused)
{
    ...
    u8 gesture_start_point[6] = {0x81,0x4D};    //手势开始数据地址
    u8 gesture_end_point[6] = {0x81,0x51};      //手势结束数据地址
    u8 gesture_width_height[6] = {0x81, 0x55};  //手势长宽数据地址
    u8 gesture_mid_point[6] = {0x81, 0x59};     //手势中心数据地址
    u8 gesture_p1_point[6] = {0x81, 0x5D};     //手势p1数据地址
    u8 gesture_p2_point[6] = {0x81, 0x61};     //手势p2数据地址
    u8 gesture_p3_point[6] = {0x81, 0x65};     //手势p3数据地址
    u8 gesture_p4_point[6] = {0x81, 0x69};     //手势p4数据地址
    ...
    #if GTP_GESTURE_WAKEUP
	  GTP_DEBUG("aaaadozestatus= %d", doze_status);
        if (DOZE_ENABLED == doze_status)
        {
		...
		gesture_echo[16] = gesture_p4_point_y;	
            ...
            {               
			   ...
			   if (0xAA == doze_buf[2])
				...
				#endif
				GTP_DEBUG("aaaenterdoze");//nyadd
				if (doze_buf[2] == 'c' && (gesture_width > 230) && (gesture_height > 230)) // x·½ÏòÏÞÖÆ width > 10mm/62.7*720=114.8; y·½ÏòÏÞÖÆ height > 10mm/111*1280=115.3; xyÍ³Ò»°´150ÏñËØµãÏÞÖÆ
					{
					GTP_DEBUG("aaatpd->gesture_enablec=:%d", tpd->gesture_enablec);//nyadd
					#if 1
					if (tpd->gesture_enablec)   
		{               GTP_DEBUG("aaatpd->gesture_enablec=c");//nyadd
						doze_status = DOZE_WAKEUP;
	                    input_report_key(tpd->dev, KEY_SCCHANGED, 1);   // lgx0913
	                    input_sync(tpd->dev);
				...
				else if (doze_buf[2] == 'v' && (gesture_width > 230) && (gesture_height > 230))
					{
					if (tpd->gesture_enablev)   
		{
                                                 GTP_DEBUG("aaatpd->gesture_enablec=v---key test#");
				...
				else if (doze_buf[2] == 'm' && (gesture_width > 230) && (gesture_height > 230))
					{
					if (tpd->gesture_enablem)   
		{
						doze_status = DOZE_WAKEUP;
				...
				else if (doze_buf[2] == 'o' && (gesture_width > 230) && (gesture_height > 230))
					{
					if (tpd->gesture_enableo)   
		{
					doze_status = DOZE_WAKEUP;
				...
				else if (doze_buf[2] == 'e' && (gesture_width > 230) && (gesture_height > 230))
					{
					if (tpd->gesture_enablee)   
		{
				...
				else if (doze_buf[2] == 'w' && (gesture_width > 230) && (gesture_height > 230))
					{
									if (tpd->gesture_enablew)   
		{
					doze_status = DOZE_WAKEUP;
				...
				else if (doze_buf[2] == 's' && (gesture_width > 230) && (gesture_height > 230))
					{
									if (tpd->gesture_enables)   
		{
					doze_status = DOZE_WAKEUP;
				...
				else if (doze_buf[2] == 'z' && (gesture_width > 230) && (gesture_height > 230))
					{
									if (tpd->gesture_enablez)   
		{
					doze_status = DOZE_WAKEUP;
