...
static ssize_t huawei_gesture_switch_write_proc(struct file *file, char __user *buff, size_t size, loff_t *ppos)
{
	...
	printk("zjh: %s, allen wy ft_wakeup_gesture = %d huawei_gesture = %ld double_gesture = %d draw_gesture = %d\n", __func__, ft_wakeup_gesture,huawei_gesture,double_gesture,draw_gesture);
	return size;
}
#endif
#endif
    ...
    u32 gesture_p4_point_x,gesture_p4_point_y;
       ...
       // check_gesture(gestrue_id);
		...
		for(i=0;i<18;i++)
		{
			...
			printk("0x%X  allen wy the pointnum = %d ", feature_codes,pointnum);
...
{	 
	...
	unsigned char monitor_buf[]={0x86,0x00};   //
#endif                                         //julian 5.16
...
#ifdef  TP_MONITOR_CTL                                 //
	i2c_master_send(i2c_client, &monitor_buf, 2);      //
#endif                                                 // julian 5.16
...
*/
	...
	__set_bit(KEY_SCCCHANGED, tpd->dev->keybit);  //liguangxian 20140913
...
static void tpd_resume( struct early_suspend *h )
{
 ...
 static void tpd_suspend( struct early_suspend *h )
 {
	...
	unsigned char sleep_buf[]={0xA5,0X03}; //modify by sven
...
/*
  ...
  printk(" gesture_enable=:%d\n",ft_wakeup_gesture);
   if(ft_wakeup_gesture){
   #ifdef  TP_MONITOR_CTL                                //
	unsigned char monitor_buf[]={0x86,0x01};          //
	i2c_master_send(i2c_client, &monitor_buf, 2);     //
	#endif			
	...
	i2c_master_send(i2c_client, &sleep_buf, 2); //modify by sven
	mutex_unlock(&i2c_access);
#endif
