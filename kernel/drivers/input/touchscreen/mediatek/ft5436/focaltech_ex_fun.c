...
static unsigned char CTPM_FW_OvTp[] = {
	#include "HQ_AL808_FT5436_OF_51.0A_20160315_app.i"
};
static unsigned char CTPM_FW_YjTp[] = {
    #include "HQ_AL806_FT5436_YJ_0x80_Ver08_20160707_app.i"
};

/*
upgrade with *.i file
*/
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client,unsigned char *CTPM_FW ,unsigned int fw_len)
{
	u8 *pbt_buf = NULL;
	int i_ret;
	...
	*/
	...
	if((CTPM_FW[0x104] ^ CTPM_FW[0x105]) == 0xFF
...
u8 fts_ctpm_get_i_file_ver(unsigned char *CTPM_FW)
{
	u16 ui_sz;
...
u8 fts_ctpm_get_i_file_vendor(unsigned char *CTPM_FW)
{
	u16 ui_sz;
	ui_sz = sizeof(CTPM_FW);
	if (ui_sz > 1)
...
//if flash damaged, force to upgrade the FW
extern void focaltech_get_upgrade_array(void);
static unsigned char ft6x36_ctpm_VidFWid_get_from_boot(  struct i2c_client *client )
{
	...
	printk("[FTS] ft6x36_ctpm_VidFWid_get_from_boot !\n");
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1
			...
			dev_err(&client->dev, "[FTS] FAIL ! Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
			printk("fts_updateinfo_curr.upgrade_id_1=%d,fts_updateinfo_curr.upgrade_id_2=%d",fts_updateinfo_curr.upgrade_id_1,fts_updateinfo_curr.upgrade_id_2);
		}
	}
...
static unsigned char ft5x46_ctpm_VidFWid_get_from_boot(  struct i2c_client *client )
{
	...
	{
	        msleep(100);
		DBG("[FTS-5x46] Step 1:Reset  CTPM\n");
		/*********Step 1:Reset  CTPM *****/
		fts_write_reg(client, 0xfc, 0xaa);
		msleep(5);
		fts_write_reg(client, 0xfc, 0x55);
		...
		/*********Step 2:Enter upgrade mode *****/
		DBG("[FTS-5x46] Step 2:Enter upgrade mode \n");
		#if 0
			auc_i2c_write_buf[0] = FT_UPGRADE_55;
			auc_i2c_write_buf[1] = FT_UPGRADE_AA;
			do {
		...
		DBG("[FTS-5x46] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0], reg_val[1]);
		if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1
			&& reg_val[1] == fts_updateinfo_curr.upgrade_id_2) {

			DBG("[FTS-5x46] Step 3 ok: CTPM ID,ID1 = 0x%x,ID2 = 0x%x ,0x%x, 0x%x:\n",
				reg_val[0], reg_val[1], fts_updateinfo_curr.upgrade_id_1, fts_updateinfo_curr.upgrade_id_2);
			break;
		} else {
			dev_err(&client->dev, "[FTS-5x46] Step 3 fail: CTPM ID,ID1 = 0x%x,ID2 = 0x%x, 0x%x, 0x%x: \n",
				reg_val[0], reg_val[1], fts_updateinfo_curr.upgrade_id_1, fts_updateinfo_curr.upgrade_id_2);
		}
#endif	
	}
	...
	auc_i2c_write_buf[0] = 0xcd;
	fts_i2c_Read(client, auc_i2c_write_buf, 1, reg_val, 1);
	DBG("[FTS-5x46]bootloader version = 0x%x\n", reg_val[0]);
	auc_i2c_write_buf[0] = 0x03;
	auc_i2c_write_buf[1] = 0x00;
	for(i = 0;i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		if (i_ret < 0)
		{
			DBG( "[FTS-5x46] Step 4: read lcm id from flash error when i2c write, i_ret = %d\n", i_ret);
			continue;
		}
...
u8* pfw_buf = NULL;//upgrade by global wangchunlong add
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
   ...
   u8 vid = 0;
	int i_ret;
    //unsigned int fw_length ;//wangchunlong
	

	fts_read_reg(client, FTS_REG_FW_VER, &uc_tp_fm_ver);// read tp fw version to uc_tp_fm_ver by wangyang
	fts_read_reg(client, FTS_REG_VENDOR_ID, &uc_tp_vendor_id);
	printk("FTS :uc_tp_vendor_id=%d \n",uc_tp_vendor_id);
	printk("FTS :chunlong:uc_tp_vendor_id=0x%x \n",uc_tp_vendor_id);
	if((0x51 != uc_tp_vendor_id) && (0x80 !=uc_tp_vendor_id))
	{
	  printk("FTS :chunlong:uc_tp_vendor_id!=0x51&0x80\n");
	  printk("[FTS] reg_module_id do not known error !\n");
	  //get Vendor ID & FW Version from CTP module' BOOT
	  ft_kick_wdt();
	  //vid = ft6x36_ctpm_VidFWid_get_from_boot( client);
	  vid = ft5x46_ctpm_VidFWid_get_from_boot(client);
  		ft_kick_wdt();
           printk("[FTS] VID= 0x%x!\n", vid);
	  if ( vid == 0xFF )
    ...
    printk("FTS >>>> fw_length=%d\n",fw_length);
	if (uc_tp_vendor_id == 0x51)
		{
		if(CTPM_FW_OvTp == NULL)
		printk("--chunlong-- CTPM_FW_OvTp[]=null\n");
		fw_length = sizeof(CTPM_FW_OvTp);
		pfw_buf = CTPM_FW_OvTp;
		uc_host_fm_ver=CTPM_FW_OvTp[fw_length-2];
		printk("--ov--:uc_host_fm_ver=0x%x\n",uc_host_fm_ver);
		printk("FTS >>>> fw_length=%d\n",fw_length);
		printk("--chunlong-- entery_OvTp:CTPM_FW_OvTp\n");
		}
	else if (uc_tp_vendor_id == 0x80)
		{
         if(CTPM_FW_YjTp == NULL)
		 printk("--chunlong-- CTPM_FW_YjTp[]=null\n");
		 fw_length = sizeof(CTPM_FW_YjTp);
		 pfw_buf = CTPM_FW_YjTp;
		 uc_host_fm_ver=CTPM_FW_YjTp[fw_length-2];
		 printk("--Yj--:uc_host_fm_ver=0x%x\n",uc_host_fm_ver);
		 printk("FTS >>>> fw_length=%d\n",fw_length);
		 printk("--chunlong-- entery_YjTp:CTPM_FW_YjTp\n");
	    }
	
	//uc_host_fm_ver = fts_ctpm_get_i_file_ver(pfw_buf);
	...
	if (/*the firmware in touch panel maybe corrupted */
		...
		dev_dbg(&client->dev, "[FTS] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",
				uc_tp_fm_ver, uc_host_fm_ver);
		i_ret = fts_ctpm_fw_upgrade_with_i_file(client,pfw_buf,fw_length);
		if (i_ret == 0)	{
			msleep(300);
			//uc_host_fm_ver = fts_ctpm_get_i_file_ver(pfw_buf);
			fts_read_reg(client, FTS_REG_FW_VER, &uc_host_fm_ver);
			ft_kick_wdt();
			  ...
			  u32 dw_lenth)
{
	...
	if(dw_lenth > 0x11f)
	{
		fw_length1 = ((u32)pbt_buf[0x100]<<8) + pbt_buf[0x101];
		printk("[FTS] dw_lenth = 0x%x,fw_length = 0x%x\n",dw_lenth,fw_length1);
		if(dw_lenth < fw_length1)
		{
...
int  fts_5x46_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1
			&& reg_val[1] == fts_updateinfo_curr.upgrade_id_2) {
			DBG("[FTS-5x46] Step 3: READ OK CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
			break;
		} else {
			dev_err(&client->dev, "[FTS-5x46] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
	...
	/*Step 4:erase app and panel paramenter area*/
	DBG("[FTS-5x46] Step 4:erase app and panel paramenter area\n");
	auc_i2c_write_buf[0] = 0x61;
	fts_i2c_Write(client, auc_i2c_write_buf, 1);	//erase app area 
	msleep(1350);
	...
	/*********Step 5:write firmware(FW) to ctpm flash*********/
	bt_ecc = 0;
	DBG("[FTS-5x46] Step 5:write firmware(FW) to ctpm flash\n");
	temp = 0;
	...
	/*********Step 6: read out checksum***********************/
	/*send the opration head */
	DBG("[FTS-5x46] Step 6: read out checksum\n");
	...
	/*********Step 7: reset the new FW***********************/
	DBG("[FTS-5x46] Step 7: reset the new FW\n");
	auc_i2c_write_buf[0] = 0x07;
	fts_i2c_Write(client, auc_i2c_write_buf, 1);
	msleep(300);	/*make sure CTP startup normally */
	ft_kick_wdt();
					...
					const char *buf, size_t count)
{
	...
	//fw_length = sizeof(CTPM_FW);//wangchunlong
    printk(">>>> fw_length=%d\n",fw_length);
	//pfw_buf = CTPM_FW;//wangchunlong
	i_ret = fts_ctpm_fw_upgrade_with_i_file(client,pfw_buf,fw_length);
	...
	const char __user *buff, unsigned long len, void *data)
{
	struct i2c_client *client = (struct i2c_client *)client01;
	int ret = 0;
	unsigned char buf[FTS_PACKET_LENGTH];
