...
#if 1
#define pbm_debug(fmt, args...) do{} while(0)
#else
...
/*HQ_pengshuihu add for HQ01936305 begin*/
extern int suspend_state;
/*HQ_pengshuihu add for HQ01936305 end*/
...
void kicker_pbm_by_gpu(bool status, unsigned int loading, int voltage)
{
	...
	struct mrp mrpmgr;
        ...
        /*HQ_pengshuihu add for HQ01936305 begin*/
        printk("wfq:kgpu s:%d,l:%d,v:%d\r\n",status,loading,voltage);
        if(3==suspend_state)
        return;
        /*HQ_pengshuihu add for HQ01936305 end*/
