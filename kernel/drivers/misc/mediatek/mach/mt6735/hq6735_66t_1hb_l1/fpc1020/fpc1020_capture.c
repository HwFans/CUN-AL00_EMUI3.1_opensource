...
/* -------------------------------------------------------------------- */
int fpc1020_write_cb_test_setup_1025(fpc1020_data_t *fpc1020, bool invert)
{
...
int fpc1020_write_cb_test_setup(fpc1020_data_t *fpc1020, bool invert)
{
	...
	switch( fpc1020->chip.type )
	{
		...
		case FPC1020_CHIP_1025X:
			error = fpc1020_write_cb_test_setup_1025(fpc1020, invert);
			break;
...
int fpc1020_capture_task(fpc1020_data_t *fpc1020)
{
	...
	if (mode == FPC1020_MODE_SINGLE_CAPTURE_CAL) {
		...
		if (error) {
			dev_err(&fpc1020->spi->dev, "%s: line=%d, error=%d\n", __func__, __LINE__, error);
			goto out_error;
		}
	}
