...
#define FPC1020_IRQ_GPIO        GPIO59|0x80000000
#define FPC1020_RESET_GPIO      GPIO60|0x80000000
#define GPIO_LENOVO_FPC_LDO_PIN GPIO200|0x80000000
/* -------------------------------------------------------------------- */
...
static irqreturn_t  fpc1020_interrupt(void);
...
extern void mtspi_set_mode(fpc1020_data_t *fpc1020,int mode);
#if 0
//add ftm test begin--
#define FPC1020_CONFIG_PROC_FILE     "fpc1020_ftm_test"
static ssize_t  ftm1020_config_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos);

static struct proc_dir_entry *ftm1020_config_proc = NULL;

static const struct file_operations ftm1020_config_ops = {
    .owner = THIS_MODULE,
    .read = ftm1020_config_read_proc,
    .write = NULL,//ftm1020_config_write_proc,
};
int fpc1020_test=1;
static ssize_t ftm1020_config_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
...
struct mt_chip_conf spi_conf_mt65xx = {
	.setuptime = 15,
	.holdtime = 15,
	.high_time = 11, //for mt6582, 104000khz/(4+4) = 130000khz
	.low_time = 11,
	.cs_idletime = 20,
	.ulthgh_thrsh = 0,
...
static int __devinit fpc1020_probe(struct spi_device *spi)
{
	...
	//mt_set_gpio_mode(GPIO_LENOVO_FPC_LDO_PIN/*115*/, GPIO_MODE_00);
	//mt_set_gpio_dir(GPIO_LENOVO_FPC_LDO_PIN/*115*/, GPIO_DIR_OUT);
	//mt_set_gpio_out(GPIO_LENOVO_FPC_LDO_PIN/*115*/, GPIO_OUT_ONE);
	//mdelay(10);
...
//<<<<<<< HEAD
#if 0
      ftm1020_config_proc = proc_create(FPC1020_CONFIG_PROC_FILE, 0666, NULL, &ftm1020_config_ops);
	if (ftm1020_config_proc == NULL)
	{
		GTP_ERROR("create_proc_entry %s failed\n", FPC1020_CONFIG_PROC_FILE);
	}
#endif     
//=======


//>>>>>>> 0dbdbf666ebf4e126da1f9eba35c90dbefc1c999
	up(&fpc1020->mutex);
...
static int fpc1020_suspend(struct device *dev)
{
	...
	//fpc1020_write_sensor_1025_setup(fpc1020);
	//if(NULL!=fpc1020)
	{
		printk("fpc1020->irq_gpio:%d",fpc1020->irq_gpio);
	}
	// set irq gpio
	...
	if (error != 0) {
		printk("mt_set_gpio_dir (eint) failed.error=%d\n",error);
          ...
          error= mt_set_gpio_pull_enable(fpc1020->irq_gpio,GPIO_PULL_ENABLE);
          error=mt_set_gpio_pull_select(fpc1020->irq_gpio,GPIO_PULL_UP);
	...
	printk("-----fpc---1-suspend----==%d fpc1020:%p fpc1020->irq:%d\n",fpc1020->worker.req_mode,fpc1020,fpc1020->irq);
	disable_irq(fpc1020->irq);// 1

	fpc1020_sleep(fpc1020, false);
	enable_irq(fpc1020->irq); // 1
					...
					struct fpc1020_platform_data *pdata)
{
	...
	mt_set_gpio_pull_enable(fpc1020->reset_gpio, GPIO_PULL_ENABLE);
...
//	fpc1020->irq = mt_gpio_to_irq(fpc1020->irq_gpio);
...
#else
#if 1
	fpc1020->irq = mt_gpio_to_irq(GPIO59);
	if (fpc1020->irq < 0) {
		printk("irq=%d\n",fpc1020->irq);
		error = fpc1020->irq;
	...
	error = request_irq(fpc1020->irq,fpc1020_interrupt,IRQF_TRIGGER_RISING,"fpc1020",fpc1020);
	if (error) {
					...
					struct fpc1020_platform_data *pdata)
{
	...
	fpc1020->cs_gpio = pdata->cs_gpio;
			...
			mt_set_gpio_mode(GPIO59|0x80000000, GPIO_MODE_01); //IRQ
		mt_set_gpio_pull_enable(GPIO59|0x80000000, GPIO_PULL_DISABLE);
	//	mt_set_gpio_mode(GPIO65|0x80000000, GPIO_MODE_01);
	//	mt_set_gpio_pull_enable(GPIO65|0x80000000, GPIO_PULL_DISABLE);
	
			mt_set_gpio_mode(GPIO66|0x80000000, GPIO_MODE_01);  //CLK
		mt_set_gpio_pull_enable(GPIO66|0x80000000, GPIO_PULL_DISABLE);
	
			mt_set_gpio_mode(GPIO67|0x80000000, GPIO_MODE_01);//IN
		mt_set_gpio_pull_enable(GPIO67|0x80000000, GPIO_PULL_DISABLE);
	
			mt_set_gpio_mode(GPIO68|0x80000000, GPIO_MODE_01);//OUT
		mt_set_gpio_pull_enable(GPIO68|0x80000000, GPIO_PULL_DISABLE);
	
		   //error= mt_set_gpio_pull_enable(fpc1020->irq_gpio,GPIO_PULL_ENABLE);
			  // error=mt_set_gpio_pull_select(fpc1020->irq_gpio,GPIO_PULL_DOWN);
		//error = mt_set_gpio_pull_enable(fpc1020->irq_gpio, GPIO_PULL_DISABLE);
#endif
					...
					char *buf)
{
	...
	case offsetof(fpc1020_diag_t, selftest):
		...
		//return 12;
		//printk("sky selftest check val=%d\r\n",(long unsigned int)val);
		break;
...
/* -------------------------------------------------------------------- */
#define CB_TYPE1_MEDIAN_BOUNDARY_VALUE_UPPER_LIMIT 215
#define CB_TYPE1_MEDIAN_BOUNDARY_VALUE_LOWER_LIMIT 185

#define CB_TYPE2_MEDIAN_BOUNDARY_VALUE_UPPER_LIMIT 115
#define CB_TYPE2_MEDIAN_BOUNDARY_VALUE_LOWER_LIMIT 85

#define ICB_TYPE1_MEDIAN_BOUNDARY_VALUE_UPPER_LIMIT 195
#define ICB_TYPE1_MEDIAN_BOUNDARY_VALUE_LOWER_LIMIT 150

#define ICB_TYPE2_MEDIAN_BOUNDARY_VALUE_UPPER_LIMIT 65
#define ICB_TYPE2_MEDIAN_BOUNDARY_VALUE_LOWER_LIMIT 20
...
static int fpc1020_check_for_deadPixels(fpc1020_data_t *fpc1020, u8* ripPixels, bool bCB)
{
	...
	int pixel_rows = fpc1020->setup.capture_row_count;
	int pixel_columns = fpc1020->setup.capture_col_groups * fpc1020->chip.adc_group_size;
	int buffersize = pixel_rows * pixel_columns;
	bool odd;
	int maxDev = 0;
	int dev1 = 0, dev2 = 0;
	int m1 = 0, m2 = 0;
	...
	sort((void *)p1, buffersize/2, sizeof(u8), &compare, NULL);  //xumin remove compile
	sort((void *)p2, buffersize/2, sizeof(u8), &compare, NULL);  //xumin remove compile
	...
	return deadpixels > FPC1020_DEADPIXEL_THRESHOLD ? 1 : 0;
}
...
static u8 fpc1020_selftest_short(fpc1020_data_t *fpc1020)
{
	...
	if(fpc1020->input.enabled) {
        ...
        dev_err(&fpc1020->spi->dev, "%s: fpc1020_read_status_reg() = 0x%x \n", __func__, error);

	fpc1020->diag.selftest = 1;
#if 1
	...
	if (error) {
		...
		fpc1020->diag.selftest=2; //sky wake up error
		goto out;
	}
	...
	if (error) {
		...
		fpc1020->diag.selftest=3;//  sky reset error
		goto out;
	}
#endif     
	error = fpc1020_check_hw_id(fpc1020);
	...
	if (error){
		fpc1020->diag.selftest=4;//  sky check id error
		goto out;
		}
	...
	if (error < 0) {
		...
		fpc1020->diag.selftest=5;//  sky capture command error
		goto out;
	}
	...
	if (error) {
		...
		fpc1020->diag.selftest=6;//  sky irq  not high after capture
		goto out;
	}
	...
	if (error) {
		...
		fpc1020->diag.selftest=7;//  sky not wait irq after capture
		goto out;
	}
	...
	if (error < 0) {
		dev_err(&fpc1020->spi->dev,
			"%s IRQ clear fail\n", id_str);
		fpc1020->diag.selftest=8;//  irq clear fail
		goto out;
	} else
		error = 0;
	...
	if (error) {
		...
		fpc1020->diag.selftest=9;//  irq not low after clear
		goto out;
	}
#if  1
	error = fpc1020_reset(fpc1020);
	...
	if (error) {
		...
		fpc1020->diag.selftest=10;//  reset fail on exit
		goto out;
	}
	...
	if (error != FPC1020_STATUS_REG_RESET_VALUE)  {
		...
		fpc1020->diag.selftest=11;//  sky  status check fail on exit
		goto out;
	}
#endif
	...
	if (error){
		...
		fpc1020->diag.selftest=12;//  dead pixels failed
		goto out;
	}
...
out:
	...
	dev_info(&fpc1020->spi->dev, "%s %s\n", id_str,
				(fpc1020->diag.selftest==1	) ? "PASS" : "FAIL");

	if (resume_input && (fpc1020->diag.selftest==1))
		fpc1020_start_input(fpc1020);
       //fpc1020_test=fpc1020->diag.selftest;
