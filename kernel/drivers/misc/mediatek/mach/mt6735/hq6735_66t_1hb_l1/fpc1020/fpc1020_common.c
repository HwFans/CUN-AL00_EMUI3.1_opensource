...
int fpc1020_write_sensor_1025_setup(fpc1020_data_t *fpc1020);
...
int fpc1020_gpio_reset(fpc1020_data_t *fpc1020)
{
	...
	mt_set_gpio_mode(fpc1020->reset_gpio/*115*/, GPIO_MODE_00);
	mt_set_gpio_dir(fpc1020->reset_gpio/*115*/, GPIO_DIR_OUT);
	//mt_set_gpio_out(GPIO_LENOVO_FPC_LDO_PIN/*115*/, GPIO_OUT_ONE);

	while (counter) {
		counter--;
                     printk("fpc1020_gpio_reset1  fpc1020->reset_gpio=%x\n",fpc1020->reset_gpio);
		mt_set_gpio_out(fpc1020->reset_gpio,GPIO_OUT_ONE);
		udelay(FPC1020_RESET_HIGH1_US);
                       printk("fpc1020_gpio_reset2  fpc1020->reset_gpio=%x\n",fpc1020->reset_gpio);
		mt_set_gpio_out(fpc1020->reset_gpio,GPIO_OUT_ZERO);
		udelay(FPC1020_RESET_LOW_US);
                           printk("fpc1020_gpio_reset3  fpc1020->reset_gpio=%x\n",fpc1020->reset_gpio);
		mt_set_gpio_out(fpc1020->reset_gpio,GPIO_OUT_ONE);
		udelay(FPC1020_RESET_HIGH2_US);
                           printk("fpc1020_gpio_reset4  fpc1020->reset_gpio=%x\n",fpc1020->reset_gpio);
...
int fpc1020_reset(fpc1020_data_t *fpc1020)
{
	...
	dev_dbg(&fpc1020->spi->dev, "%s\n", __func__);
	dev_dbg(&fpc1020->spi->dev, " %s  fpc1020->soft_reset_enabled:%d\n", __func__,fpc1020->soft_reset_enabled);
...
int fpc1020_check_hw_id(fpc1020_data_t *fpc1020)
{
	...
	if(fpc1020->force_hwid > 0) {
		...
		printk("--fpc-- Modify hwid from 0x%x\n", hardware_id);
	...
	if (match) {
		...
		printk("fpc sky1  Hardware id: 0x%x (%s, rev.%d) \n",hardware_id,
						chip_text[fpc1020->chip.type],
						fpc1020->chip.revision);
	} else {
		dev_err(&fpc1020->spi->dev,
			"Hardware id mismatch: got 0x%x\n", hardware_id);
                     printk("fpc sky2 Hardware id mismatch: got 0x%x\n",hardware_id);
...
void mtspi_set_mode(fpc1020_data_t *fpc1020,int mode)
{
...
int fpc1020_wake_up(fpc1020_data_t *fpc1020)
{
	...
	int status = fpc1020_read_status_reg(fpc1020);
           printk("fpc1020_wake_up--check  %d   %d  %d   %d\r\n",reset,status,(fpc1020_status_reg_t)(status & status_mask),FPC1020_STATUS_REG_IN_IDLE_MODE);
	if (reset == 0 && status >= 0 &&
...
int fpc1020_sleep(fpc1020_data_t *fpc1020, bool deep_sleep)
{
	...
	int mode =0;
	const fpc1020_status_reg_t status_mask = FPC1020_STATUS_REG_MODE_MASK;
	mode = FPC1020_CMD_ACTIVATE_SLEEP_MODE;//(deep_sleep) ? FPC1020_CMD_ACTIVATE_DEEP_SLEEP_MODE :
