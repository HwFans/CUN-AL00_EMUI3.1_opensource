...
u32 pinSet[3][8] = {
    ...
    if (On) {
		...
		else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV8855_MIPI_RAW, currSensorName)))
		{
		        #if 1
                ISP_MCLK1_EN(1);
				mdelay(2);  
	           ...
	           mdelay(1);
	            #endif
	            		PK_DBG("[CAMERA LENS] 8855 2\n");
	        }
 else if ((currSensorName && (0 == strcmp(currSensorName,"hi843bmipiraw"))))
        {
				//First Power Pin low and Reset Pin Low
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
 ...
 // end of HI-843B Power ON Sequence 
        ...
        else  if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_SP2509_RAW, currSensorName)))
        {
        	PK_DBG("[CAMERA LENS] sp2509 1\n");
    ...
    // HI-843B Power Off Sequence 
    else if (currSensorName && (0 == strcmp(currSensorName,"hi843bmipiraw")))
        {
            //reset pull down
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
			PK_DBG("RST ON to OFF\n");
				if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMRST],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_MODE]))
					{PK_DBG("[CAMERA SENSOR] set gpio mode failed!! \n");}
				if(mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_CMRST],GPIO_DIR_OUT))
					{PK_DBG("[CAMERA SENSOR] set gpio dir failed!! \n");}
            ...
            //VCAM_D
            if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_D, mode_name)) {
                PK_DBG("[CAMERA SENSOR] Fail to OFF digital power (VCAM_D), power id = %d \n", CAMERA_POWER_VCAM_D);
                //return -EIO;
                goto _kdCISModulePowerOn_exit_;
            }
            ...
            if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_A,mode_name))
            {
                 PK_DBG("[CAMERA SENSOR] Fail to OFF core power (VCAM_A), power id = %d \n",CAMERA_POWER_VCAM_A);
                 goto _kdCISModulePowerOn_exit_;
            }

            //VCAM_IO
            if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_IO,mode_name)) {
                PK_DBG("[CAMERA SENSOR] Fail to OFF analog power (VCAM_IO), power id= (%d) \n", CAMERA_POWER_VCAM_IO);
                //return -EIO;
                goto _kdCISModulePowerOn_exit_;
            }

        }
// end of hi843b power off

      
  
        else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_GC2355_MIPI_RAW, currSensorName)))
        {
        	PK_DBG("[CAMERA LENS] 2355 off 1\n");
            //mt_set_gpio_out(GPIO_SPI_MOSI_PIN,GPIO_OUT_ZERO);
            //Set Power Pin low and Reset Pin Low
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! (CMPDN)\n");}
            ...
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
            	...
            	PK_DBG("[CAMERA LENS] 2355 off 2\n");

        }
        else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_SP2509_RAW, currSensorName)))
        {
        	PK_DBG("[CAMERA LENS] sp2509 off 1\n");
            //mt_set_gpio_out(GPIO_SPI_MOSI_PIN,GPIO_OUT_ZERO);
            //Set Power Pin low and Reset Pin Low     	
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMRST],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_MODE])){PK_DBG("[CAMERA SENSOR] set gpio mode failed!! (CMRST)\n");}
          ...
          if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_IO, mode_name)) {
              ...
              goto _kdCISModulePowerOn_exit_;
               	...
               	PK_DBG("[CAMERA LENS] sp2509 off 2\n");
