...
static void vibrator_enable(struct timed_output_dev *dev, int value)
{
	...
	//add by zym,2015-04-01,enhance vibrator experience
	unsigned long vib_mode =0;
	...
	//add by zym,2015-04-01,enhance vibrator experience
                if(value !=0)
                {
                  ...
                  {
                    vibr_short_power_set();
                    hw = mt_get_cust_short_vibrator_hw();
                  } 
                }

                value = value>>2;
                printk("[vibrator]vibrator_enable: vibrator first in value = %d,vib_mode=%ld\n", value,vib_mode);
...
static void vib_shutdown(struct platform_device *pdev)
{
	...
	spin_unlock_irqrestore(&vibe_lock, flags);///zym
}
