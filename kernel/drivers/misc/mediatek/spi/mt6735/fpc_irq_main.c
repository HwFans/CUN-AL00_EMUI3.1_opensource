...
#define GPIO_LENOVO_FPC_EINT_PIN         (GPIO59 | 0x80000000)
...
static int fpc_irq_get_of_pdata(struct platform_device *dev,
				fpc_irq_pdata_t *pdata)
{
	pdata->irq_gpio = GPIO_LENOVO_FPC_EINT_PIN;
	pdata->irq_no   = EINT59;
	pdata->rst_gpio = GPIO_LENOVO_FPC_RESET_PIN;
				...
				fpc_irq_pdata_t *pdata)
{
	...
	fpc_irq_data->pdata.irq_no =pdata->irq_no;
  	error = mt_set_gpio_mode(pdata->irq_gpio, GPIO_LENOVO_FPC_EINT_PIN_M_EINT);
	if (error != 0) {
	...
	mt_eint_set_sens(pdata->irq_no, CUST_EINT_EDGE_SENSITIVE);
	mt_eint_set_polarity(pdata->irq_no, 1);
	mt_eint_set_hw_debounce(pdata->irq_no, 1);
