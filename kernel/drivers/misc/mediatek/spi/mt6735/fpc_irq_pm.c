...
#define FPC_IRQ_KEY_ANDR_BACK	KEY_FPC//modified for fingerprint by liuchaochao
...
int fpc_irq_pm_init(fpc_irq_data_t *fpc_irq_data)
{
	...
	fpc_irq_data->pm.supply_on      = false;
	fpc_irq_data->pm.hw_reset       = false;
	fpc_irq_data->pm.notify_enabled = false;
	fpc_irq_data->pm.wakeup_req     = false;
	...
	if (!ret) {
		...
		set_bit(FPC_IRQ_KEY_ANDR_BACK, fpc_irq_data->input_dev->keybit);
...
int fpc_irq_click_event(fpc_irq_data_t *fpc_irq_data, int val)
{
	...
	input_report_key(fpc_irq_data->input_dev, FPC_IRQ_KEY_ANDR_BACK, 1);
	input_report_key(fpc_irq_data->input_dev, FPC_IRQ_KEY_ANDR_BACK, 0);
