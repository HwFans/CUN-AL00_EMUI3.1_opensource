    ...
    for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
        ...
        if (sensorID == 0) {    /* not implement this feature ID */
            ...
            snprintf(mtk_ccm_name,sizeof(mtk_ccm_name),"%s CAM[%d]:%s;",mtk_ccm_name,g_invokeSocketIdx[i],g_invokeSensorNameStr[i]);
                        if (DUAL_CAMERA_MAIN_SENSOR == g_invokeSocketIdx[i])
                                   {
                                   	PK_DBG("main sensor:%s\n", g_invokeSensorNameStr[i]);
                                     memcpy(hardware_info.mainCameraName, (char *)g_invokeSensorNameStr[i], sizeof(g_invokeSensorNameStr[i]));
                                   }
                                   else if (DUAL_CAMERA_SUB_SENSOR == g_invokeSocketIdx[i]) 
                                   {
                                      PK_DBG("sub sensor:%s\n", g_invokeSensorNameStr[i]);
                                     memcpy(hardware_info.subCameraName, (char *)g_invokeSensorNameStr[i], sizeof(g_invokeSensorNameStr[i]));
                                   }
            err = ERROR_NONE;
        }
        if (ERROR_NONE != err)
...
inline static int  adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
    ...
    /*  */
	//[HQ00055994] wwx336983/wangguan add for camera ke when feature control SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO begin
	#if 0
    if (g_pSensorFunc) {
	...
	#else
   if ( (SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY ==pFeatureCtrl->FeatureId) || (SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO == pFeatureCtrl->FeatureId))
	{
		MUINT32 *pValue = NULL;
	...
	#endif
	//[HQ00055994] wwx336983/wangguan add for camera ke when feature control SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO end
...
static int CAMERA_HW_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    /* set I2C clock rate */
    g_pstI2Cclient->timing = 300;/* 100k */
