...
static imgsensor_info_struct imgsensor_info = {
	...
	.checksum_value = 0xc3dd012b,	//0xc3dd012b,
    ...
    .cap_delay_frame = 2,        //3enter capture delay frame num
...
///////////////////////////////////////////////////////OTP Code Start 
////////////////////////////////////////////////
#define Typical_RG 0x320
#define Typical_BG 0x30A
...
static kal_uint16 read_eeprom(u16 EEpromaddr, kal_uint32 addr)
{
	kal_uint16 get_byte=0;
...
iReadRegI2C(&puSendCmd, 1, (u8*)&get_byte, 1, (EEpromaddr&0XFF));//EEPROM地址为8bit; 

return get_byte&0x00ff;

}

static void APPLYAWB(kal_uint16 rg,kal_uint16 bg,kal_uint16 golden_rg,kal_uint16 golden_bg) //278 317  272 315
{

    kal_uint16 r_ratio=0,b_ratio=0;
    kal_uint16 R_GAIN=0,B_GAIN=0,Gr_GAIN=0,Gb_GAIN=0,G_GAIN=0;
    
    LOG_INF("O-film APPLYAWB R/G=%X ,B/G=%X , Typical_R/G=%X , Typical_B/G=%X \n",rg,bg,golden_rg,golden_bg);
    
    r_ratio=512*(golden_rg)/(rg); // 501
    b_ratio=512*(golden_bg)/(bg);//509
    ...
    LOG_INF("O-film APPLYAWB R_GAIN = %d",R_GAIN);
    LOG_INF("O-film APPLYAWB B_GAIN = %d",B_GAIN);
    LOG_INF("O-film APPLYAWB G_GAIN = %d",G_GAIN);

		if(R_GAIN>GAIN_DEFAULT)
			{
				write_cmos_sensor1(GAIN_RED_ADDR,R_GAIN);
			}
    if(B_GAIN>GAIN_DEFAULT)
			{
				write_cmos_sensor1(GAIN_BLUE_ADDR, B_GAIN); 
			}
			
		if(G_GAIN>GAIN_DEFAULT)
			{

				write_cmos_sensor1(GAIN_GREEN1_ADDR, G_GAIN);
				write_cmos_sensor1(GAIN_GREEN2_ADDR, G_GAIN);
			}
}

static void UpdateAWB()
{
	kal_uint16 AWBData[16] = {0};
	int i=0;
	kal_uint16 RGain =0,BGain=0;
	
	for( i = 0; i < 16; i++ )
	{
		AWBData[i] = read_eeprom(0xA0, i);
		LOG_INF("O-FILM AWBdata[%X] = %X \n", i, AWBData[i]);
	}

	kal_uint16 module_id = AWBData[4];
	LOG_INF("O-film module_id = %d",module_id);
  
	RGain = (AWBData[10]<<8) + AWBData[11];
  BGain = (AWBData[12]<<8) + AWBData[13];

	LOG_INF("O-film R/G value = %x \n",RGain);
	LOG_INF("O-film B/G value = %x \n",BGain);

	APPLYAWB(RGain, BGain ,Typical_RG,Typical_BG);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x0101, 0x00);  //mirror03
  write_cmos_sensor1(0x6028, 0xD000);
