...
#include "ov5670_hq_mipi_Sensor.h"
...
#define GPIO_SUBCAM_ID (GPIO123|0x80000000)
static DEFINE_SPINLOCK(imgsensor_drv_lock);



static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV5670_HQ_MIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
	...
	.i2c_addr_table = {0x6c, 0x6c, 0xff},//record sensor support all write id addr, only supprt 4must end with 0xff
};
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x0302, 0xd2); //;69
	...
	write_cmos_sensor(0x030d, 0x3c);//0x1e 
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
                        *sensor_id += 1; //niyangadd
			   if (mt_get_gpio_in(GPIO_SUBCAM_ID) == 1) {
                          subcamid=1;
                       printk("[++++ov5670hq_adc++++]subcamid = %d\n", subcamid);
                              } 
			if (*sensor_id == imgsensor_info.sensor_id && subcamid ==1) {			
				LOG_INF("HQi2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}	
			LOG_INF("HQRead sensor id fail, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
//printk("aaaaahqsubcamid = %d\n", mt_get_gpio_in(GPIO_SUBCAM_ID));
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	preview_setting();
set_mirror_flip(imgsensor.mirror);//nyadd
	return ERROR_NONE;
}	/*	preview   */
...
UINT32 OV5670_HQ_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
