...
//    ASSERT(prQM->u4TimeToAdjust && prQM->u4TimeToUpdateQueLen);
    ...
    DBGLOG(QM, INFO, ("QM: NIC_TX_BUFF_COUNT_TC0 = %d\n", NIC_TX_BUFF_COUNT_TC0));
    DBGLOG(QM, INFO, ("QM: NIC_TX_BUFF_COUNT_TC1 = %d\n", NIC_TX_BUFF_COUNT_TC1));
    DBGLOG(QM, INFO, ("QM: NIC_TX_BUFF_COUNT_TC2 = %d\n", NIC_TX_BUFF_COUNT_TC2));
    DBGLOG(QM, INFO, ("QM: NIC_TX_BUFF_COUNT_TC3 = %d\n", NIC_TX_BUFF_COUNT_TC3));
    DBGLOG(QM, INFO, ("QM: NIC_TX_BUFF_COUNT_TC4 = %d\n", NIC_TX_BUFF_COUNT_TC4));
    DBGLOG(QM, INFO, ("QM: NIC_TX_BUFF_COUNT_TC5 = %d\n", NIC_TX_BUFF_COUNT_TC5));
...
BOOLEAN
qmPopOutDueToFallWithin(P_ADAPTER_T prAdapter,
    IN P_RX_BA_ENTRY_T prReorderQueParm,
    OUT P_QUE_T prReturnedQue,
...
VOID
qmPopOutDueToFallAhead(P_ADAPTER_T prAdapter,
    IN P_RX_BA_ENTRY_T prReorderQueParm,
    OUT P_QUE_T prReturnedQue
    )
{
	...
	if (arpOpCode == ARP_PRO_RSP) {
		...
		if (prBssInfo && prBssInfo->prStaRecOfAP && prBssInfo->prStaRecOfAP->aucMacAddr) {
			if (EQUAL_MAC_ADDR(&(pucData[ETH_TYPE_LEN_OFFSET + 10]), /* source hardware address */
				...
				kalMemCopy(apIp, &(pucData[ETH_TYPE_LEN_OFFSET + 16]), sizeof(apIp)); /* src ip address */
