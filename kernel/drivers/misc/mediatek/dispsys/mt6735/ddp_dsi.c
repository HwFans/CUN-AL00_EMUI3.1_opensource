...
DSI_STATUS DSI_TXRX_Control(DISP_MODULE_ENUM module, cmdqRecHandle cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	bool hstx_cklp_en = dsi_params->cont_clock?TRUE:FALSE;        //stone_add 
...
static void lcm_mdelay(UINT32 ms)
{
	...
	{
		//add by caoxuhao for HQ01458405
		mdelay(ms);
		//msleep(ms);
		//udelay(ms*1000);
	}
...
int ddp_dsi_stop(DISP_MODULE_ENUM module, void *cmdq_handle)
{
	int i =0;
	unsigned int tmp = 0;
	unsigned long long start_time,end_time; //mtk add
	DISPFUNC();
	...
	if(_dsi_is_video_mode(module))
	{
		...
		i = DSI_MODULE_BEGIN(module);
		start_time = sched_clock(); //mtk add
		while(1)
		{		
			end_time = sched_clock(); //mtk add	
			tmp = INREG32(&DSI_REG[i]->DSI_INTSTA);
			if(!(tmp &0x80000000)) break;
			
			//mtk add begin
            if((end_time - start_time)/1000 > 2000000)// 2s timeout 
		    {
				DISPERR("dsi wait not busy timeout,start at %lld, end at %lld\n", start_time, end_time);
			    DSI_DumpRegisters(module,1);
			    DSI_Reset(module, NULL);
...
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	else if(state == CMDQ_ESD_CHECK_CMP)
	{
		...
		for(i = 0; i < 3; i++)
		{
			...
			DISPDBG("[DSI]short enter cmp check_table cmd=0x%x,count=0x%x,para_list[0]=0x%x,para_list[1]=0x%x\n",dsi_params->lcm_esd_check_table[i].cmd,dsi_params->lcm_esd_check_table[i].count,dsi_params->lcm_esd_check_table[i].para_list[0],dsi_params->lcm_esd_check_table[i].para_list[1]);
			DISPDBG("[DSI]long enter cmp check_table cmd=0x%x,count=0x%x,para_list[0]=0x%x,para_list[1]=0x%x\n",dsi_params->lcm_esd_check_table[i].cmd,dsi_params->lcm_esd_check_table[i].count,dsi_params->lcm_esd_check_table[i].para_list[0],dsi_params->lcm_esd_check_table[i].para_list[1]);
			...
			int tmp_i=0, tmp_j=0, condition_cnt=0;
			/* calculate how many conditions it has */
			DISPCHECK("sizeof para_list[] is %ld\n", sizeof(dsi_params->lcm_esd_check_table[i].para_list));
	

						for (tmp_i=0; tmp_i<sizeof(dsi_params->lcm_esd_check_table[i].para_list); tmp_i++) {
				if (0 == dsi_params->lcm_esd_check_table[i].para_list[tmp_i])
					break;
			}
			condition_cnt = tmp_i / dsi_params->lcm_esd_check_table[i].count;
			if (condition_cnt > 1)
				DISPCHECK("[DSI]Cmd: 0x%x has [%d] conditions\n", dsi_params->lcm_esd_check_table[i].cmd, condition_cnt);

			/* cmp */
			for (tmp_i=0; tmp_i<condition_cnt; tmp_i++) {
				for (tmp_j=0; tmp_j<dsi_params->lcm_esd_check_table[i].count; tmp_j++) {
					DISPCHECK("[DSI]cmp 0x%x to 0x%x\n", *(&read_data1.byte0+tmp_j), dsi_params->lcm_esd_check_table[i].para_list[tmp_j+tmp_i*dsi_params->lcm_esd_check_table[i].count]);
					if (*(&read_data1.byte0+tmp_j) != dsi_params->lcm_esd_check_table[i].para_list[tmp_j+tmp_i*dsi_params->lcm_esd_check_table[i].count])
						break;
				}

				if (tmp_j == dsi_params->lcm_esd_check_table[i].count) {
					/* one condition compared, break */
					ret = 0;	// esd pass
					break;
				} else
					ret = 1;	//  esd fail
