...
BOOTMODE g_boot_mode_ex __nosavedata = UNKNOWN_BOOT;//lupingzhong_add huaqin factory mode
...
static int __init dt_get_boot_common(unsigned long node, const char *uname, int depth, void *data)
{
    ...
    printk("lpz %s g_boot_mode=%d, g_boot_mode_ex=%d\n", __func__, g_boot_mode, g_boot_mode_ex);
	/* break now */
	return 1;
}
#endif
...
//lupingzhong_add huaqin factory mode start
BOOTMODE get_boot_mode_ex(void)
{
    return g_boot_mode_ex;
}
//lupingzhong_add huaqin factory mode end
...
static int boot_status_proc_show(struct seq_file *p, void *v)
{
	switch (g_boot_mode_ex) {
	case HQ_FACTORY_BOOT:
                printk("enter hq factory boot\n");
		seq_puts(p, "1\n");
		break;
	default:
                printk("don't enter hq factory boot\n");
		seq_puts(p, "0\n");
		break;
	}
...
static int boot_status_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, boot_status_proc_show, NULL);
}


//lupingzhong add for factory APK start
#define PROC_NAME1 "boot_status"
static struct proc_dir_entry *proc_boot_entry;

static ssize_t boot_status_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
        unsigned char factory_flag;
        int ret = 1;
        if (HQ_FACTORY_BOOT== get_boot_mode_ex()){
                factory_flag= 1;
        } else {
                factory_flag = 0;
        }
        printk("lpz %s g_boot_mode=%d, g_boot_mode_ex=%d\n", __func__, g_boot_mode, g_boot_mode_ex);
        ret = copy_to_user(buf, &factory_flag, sizeof(factory_flag));
        if (ret)
                printk("lpz factory flag copy to user failed\n");
        else
                printk("lpz factory flag copy to user success\n");
        ret = sizeof(factory_flag);
        return ret;
}

static const struct file_operations boot_status_proc_fops = {
	//.open = boot_status_proc_open,
	//.read = seq_read,
	//.llseek = seq_lseek,
	//.release = single_release,
    .read = boot_status_read,
};
...
int create_proc_boot_status(void)
{
		//CPM2016070500216 change 0777 to 0774 gaochao 160708 begin
        proc_boot_entry = proc_create(PROC_NAME1, 0774, NULL, &boot_status_proc_fops);
        //CPM2016070500216 gaochao 160708 end
        if (NULL == proc_boot_entry) {
                return -ENOMEM;
        } else {
                printk("create factory proc entry success");
        }
        return 0;
}
//lupingzhong end.
...
static int __init boot_common_core(void)
{
        ...
        create_proc_boot_status();// lupingzhong add for factory mode
...
static int __init boot_common_init(void)
{
	...
	printk("lpz boot_common_init boot_mode = %d, state(%d,%d,%d)", g_boot_mode, 
             atomic_read(&g_boot_init), atomic_read(&g_boot_errcnt),
             atomic_read(&g_boot_status));
	return 0;
}
...
EXPORT_SYMBOL(get_boot_mode_ex);//lupingzhong_add huaqin factory mode
