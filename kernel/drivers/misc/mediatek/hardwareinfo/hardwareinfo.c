...
//add begin by gongzibiao 20160602 for CPM2016052300323 add for wifi addr
#include <linux/types.h>
#include <linux/module.h>
#include <linux/fs.h>
...
#endif
//add end by gongzibiao 20160602 for CPM2016052300323 add for wifi addr
...
typedef struct _MT6620_CFG_PARAM_STRUCT {
    ...
    char             u2Part1OwnVersion[2];
    char             u2Part1PeerVersion[2];
    char              aucMacAddress[6];
    char              aucCountryCode[2];
} WIFI_CFG_PARAM_STRUCT;
static int nvram_read(char *filename, char *buf, ssize_t len, int offset)
{
        printk("[MT6620][nvram_read] wifi\n");
        struct file *fd;
        int retLen = -1;
        ...
        if (IS_ERR(fd)) {
                printk("[MT6620][nvram_read] : failed to open!!\n");
                return -1;
        }

        do {
                if ((fd->f_op == NULL) || (fd->f_op->read == NULL)) {
                        printk("[MT6620][nvram_read] : file can not be read!!\n");
                        break;
                }
                ...
                if (fd->f_pos != offset) {
                        if (fd->f_op->llseek) {
                                if (fd->f_op->llseek(fd, offset, 0) != offset) {
                                        printk("[MT6620][nvram_read] : failed to seek!!\n");
                                        break;
                                }
                        } else {
                                fd->f_pos = offset;
...
static ssize_t show_wifi_addr(struct device *dev,struct device_attribute *attr, char *buf)
{
    int ret_value = 1;
    WIFI_CFG_PARAM_STRUCT wifi_para;
    ret_value = nvram_read(WIFI_NVRAM_FILE_NAME,(char *)&wifi_para, sizeof(WIFI_CFG_PARAM_STRUCT), 0);
    printk("nvram_read wifi addr:0x%0x,0x%0x,0x%0x,0x%0x,0x%0x,0x%0x,",wifi_para.aucMacAddress[0],wifi_para.aucMacAddress[1],wifi_para.aucMacAddress[2],wifi_para.aucMacAddress[3],wifi_para.aucMacAddress[4],wifi_para.aucMacAddress[5]);
    if (ret_value != -1) {
			ret_value = sprintf(buf, "%02x:%02x:%02x:%02x:%02x:%02x\n",wifi_para.aucMacAddress[0],wifi_para.aucMacAddress[1],wifi_para.aucMacAddress[2],wifi_para.aucMacAddress[3],wifi_para.aucMacAddress[4],wifi_para.aucMacAddress[5]);
    } else {
            ret_value = sprintf(buf, "WIFI MAC ADDR NOT FOUND\n");
    }
...
*/
    ...
    if(hardware_info.mainCameraName) { 
        ...
        } else if (!strcmp(hardware_info.mainCameraName, "ov8855mipiraw")){
    ...
    if(hardware_info.subCameraName){
        if (!strcmp(hardware_info.subCameraName, "gc2355mipiraw")) {
            ...
            count += sprintf(buf+count, "Q-Tech "); 
			count += sprintf(buf+count, "%s %s\n", hardware_info.subCameraName, "FF 5M F2.2");
...
//add by gongzibiao 20160602 for CPM2016052300323 add for wifi addr
static DEVICE_ATTR(hq_wifiaddr, 0644, show_wifi_addr, NULL);
...
//    if((ret_device_file = device_create_file(&(dev->dev), &dev_attr_04_flash)) != 0) goto exit_error;
	...
	//add by gongzibiao 20160602 for CPM2016052300323 add for wifi addr
    if((ret_device_file = device_create_file(&(dev->dev), &dev_attr_hq_wifiaddr)) != 0) goto exit_error;
...
static int HardwareInfo_driver_remove(struct platform_device *dev)
{
	...
	//add begin by gongzibiao 20160602 for CPM2016052300323 add for wifi addr
	device_remove_file(&(dev->dev), &dev_attr_hq_hardwareinfo);
    device_remove_file(&(dev->dev), &dev_attr_hq_manfinfo); 
    device_remove_file(&(dev->dev), &dev_attr_hq_wifiaddr);
	//add end by gongzibiao 20160602 for CPM2016052300323 add for wifi addr
    return 0;
}
