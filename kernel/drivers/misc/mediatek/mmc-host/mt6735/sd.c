...
#ifdef CONFIG_HUAWEI_KERNEL
	HUAWEI_DBG("HUAWEI %s: starting CMD%u arg %08x flags %08x \n", mmc_hostname(host->mmc),mrq->cmd->opcode,mrq->cmd->arg,mrq->cmd->flags);
#endif
...
static irqreturn_t msdc1_eint_handler(void)
{
	...
	if(!(host->hw->cd_level ^ host->sd_cd_polarity))
	{
		printk("1_power_off_first\n"); 
		//pmic_config_interface_nolock(0x0A20,0x0,0x1,1);
		pmic_config_interface_nolock(0x0A1C,0x0,0x1,1);
	}
	//end by zym
    return IRQ_HANDLED;
}
...
/*
	...
	dentry_mmclog= debugfs_create_dir("hw_mmclog", NULL);
        if(dentry_mmclog)
        	{
		debugfs_create_file("rwlog_enable", S_IFREG |S_IRWXU |S_IRGRP |S_IROTH ,dentry_mmclog, NULL, &rwlog_enable_fops);
		debugfs_create_file("rwlog_index", S_IFREG|S_IRWXU|S_IRGRP|S_IROTH,dentry_mmclog, NULL, &rwlog_index_fops);
		debugfs_create_file("debug_mask", S_IFREG|S_IRWXU|S_IRGRP|S_IROTH,dentry_mmclog, NULL, &debug_mask_fops);
		}
