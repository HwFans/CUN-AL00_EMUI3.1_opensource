...
#if (defined(CONFIG_NEUSOFT_TZ_DRIVER))
#define RPMB_DATA_BUFF_SIZE (1024 * 33)
static unsigned char *rpmb_buffer = NULL;
#endif    
...
/** 
 * 功能描述. 
 * @param[in]   	[card] emmc card info. 
 * @param[out]  	[wc] read rpmb write counter .  
 * @par 修改日志 
 *      libaojun于2015-04-24创建 
 */ 
...
int neu_rpmb_req_read_data(struct mmc_card *card, struct s_rpmb *param,u32 blk_cnt)//struct mmc_card *card, 
{
    ...
    //hmac_sha256(rpmb_key, 32, param->data, 284 , hmac);
    //~ for( j = 0;j<32;j++)
	//~ {
		//~ printk(" rpmb_frame.mac[%d] = %x\n",j,hmac[j]);
	//~ }	
   //~ 
	return ret;
}
 ...
 * 功能描述. 
 * @param[in]   [card] emmc card info. 
 * @param[in]  	[param] write data info.   
 * @par 修改日志 
 *      libaojun于2015-04-24创建 
 */ 
int neu_rpmb_req_write_data(struct mmc_card *card, struct s_rpmb *param,u32 blk_cnt)//struct mmc_card *card, 
{
    ...
    //memset(param->nonce,0,RPMB_SZ_NONCE);
    //memset(param->mac,0,RPMB_SZ_MAC);
...
{    
    //enum mc_result  mc_ret;
    MSG(INFO, "%s, !!!!!!!!!!!!\n", __func__);
//begin, [microtrust] [libaojun] [add] [For TEEI CMD][2015.07.24]     
#if (defined(CONFIG_NEUSOFT_TZ_DRIVER))
...
#endif  
//end, [microtrust] [libaojun] [add] [For TEEI CMD][2015.07.24] 
...
//begin, [microtrust] [libaojun] [add] [For TEEI CMD][2015.04.24]  
#if (defined(CONFIG_NEUSOFT_TZ_DRIVER))
	...
	if((cmd == RPMB_IOCTL_SOTER_WRITE_DATA) || (cmd == RPMB_IOCTL_SOTER_READ_DATA))    
		{   
			...
			rpmbinfor.size =  *(unsigned char *)rpmb_buffer | (*((unsigned char *)rpmb_buffer + 1) << 8) \
			             | (*((unsigned char *)rpmb_buffer+2) << 16) | (*((unsigned char *)rpmb_buffer+3) << 24);
...
#endif
//end, [microtrust] [libaojun] [add] [For TEEI CMD][2015.04.24] 
    switch (cmd) {
        ...
        case RPMB_IOCTL_WRITE_DATA:
            ...
            break;  
//begin, [microtrust] [libaojun] [add] [For TEEI CMD Process][2015.04.24] 
#if (defined(CONFIG_NEUSOFT_TZ_DRIVER))                  
		case RPMB_IOCTL_SOTER_WRITE_DATA:
            //MSG(INFO, "%s, cmd = RPMB_IOCTL_SOTER_WRITE_DATA!\n", __func__);
			ret = neu_rpmb_req_write_data(mtk_msdc_host[0]->mmc->card, (struct s_rpmb *)(rpmbinfor.data_frame), rpmbinfor.size/1024);
			if (ret) {
					MSG(ERR, "%s, emmc_rpmb_req_handle IO error!!!(%x)\n", __func__, ret);
				}
		...
		case RPMB_IOCTL_SOTER_READ_DATA:
            //MSG(INFO, "%s, cmd = RPMB_IOCTL_SOTER_READ_DATA!\n", __func__);
			ret = neu_rpmb_req_read_data(mtk_msdc_host[0]->mmc->card, (struct s_rpmb *)(rpmbinfor.data_frame), rpmbinfor.size/1024);
			if(ret) {
				MSG(ERR, "%s, emmc_rpmb_req_handle IO error!!!(%x)\n", __func__, ret);
			}
			copy_to_user((void*)arg, rpmb_buffer, 4 + rpmbinfor.size);
        ...
        case RPMB_IOCTL_SOTER_GET_CNT:
			//MSG(INFO, "%s, cmd = RPMB_IOCTL_SOTER_GET_CNT!\n", __func__);
			ret = neu_rpmb_req_get_wc(mtk_msdc_host[0]->mmc->card, (unsigned int *)arg);
			break;
#endif			
//end, [microtrust] [libaojun] [add] [For TEEI CMD Process][2015.04.24]			
        default:
