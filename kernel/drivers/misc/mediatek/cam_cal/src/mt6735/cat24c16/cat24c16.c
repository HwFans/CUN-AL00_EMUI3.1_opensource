...
CAM_CALERR(" aaaaaaaaaaAddr = 0x%x, Addr = 0x%x\n", g_pstI2Cclient->addr,a_pSendData[0]);
    spin_unlock(&g_CAM_CALLock);
...
//	CAM_CALDB("selective_read_byte\n");
	...
	else if((addr >= 0xA200)&&(addr <= 0xA2FF ))
		{
			Device_id = 0xA2;		
		}
	else if((addr >= 0xA400)&&(addr <= 0xA4FF))
		{
			Device_id = 0xA4;
		}
	else if((addr >= 0xA600)&&(addr <= 0xA6FF))
		{
			Device_id = 0xA6;
		}
	else
		{
		    //g_pstI2Cclient->addr = s5k3l2OTP_LSC1_DEVICE_ID>>1;
			Device_id = 0xA0;
			CAM_CALDB("[CAM_CAL] Note In LSC Block1 Info a_u2Addr is %x \n",addr);
		}
	if(iReadRegI2C(&offset, 1, (u8*)data, 1,Device_id) ){//i2c_id+(page<<1))<0
...
int selective_read_region(u32 addr, BYTE* data,u16 i2c_id,u32 size)
{
  //  u32 page = addr/PAGE_SIZE; /* size of page was 256 */
	...
	//kdSetI2CSpeed(EEPROM_I2C_SPEED);//nyadd
	int ret = 0;
...
#endif
{
    ...
    switch(a_u4Command)
    {
        ...
        case CAM_CALIOC_G_READ:
            CAM_CALDB("[S24CAM_CAL] aaaaRead CMD \n");
#ifdef CAM_CALGETDLT_DEBUG
            do_gettimeofday(&ktv1);
#endif
...
{
    CAM_CALDB("[S24CAM_CAL] aaaabCAM_CAL_Open\n");
    spin_lock(&g_CAM_CALLock);
    if(g_u4Opened)
    {
        spin_unlock(&g_CAM_CALLock);
