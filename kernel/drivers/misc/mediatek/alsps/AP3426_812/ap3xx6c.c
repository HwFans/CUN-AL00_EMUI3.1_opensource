...
/*******************************************/
    ...
    ap3xx6_set_PSgain(client, 0x03); //ps gain
...
static int ap3xx6_Calibration_every_time(struct i2c_client *client)
{
    ...
    if(abs(ui_ps_data - gi_ps_raw_data) > DI_PS_CAL_DIFF)
    {
...
static void ap3xx6_eint_work(struct work_struct *work)
{
	...
	printk("wmy sensor_data.values[0] %d--%d\n", sensor_data.values[0], __LINE__);  //add by wmy
...
static int alsps_get_corss_talk(struct ap3xx6_priv *obj, int count)
{
	...
	ps_data_average = ps_data_total / count;
...
static long ap3xx6_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ALSPS_GET_CALIBRATION_DATA:
			...
			printk("kaka before hq_alsps_get_alsps_cali:   ps_thd_val_high:%d ps_thd_val_low:%d\n"
                        ,atomic_read(&obj->ps_thd_val_h)
                        ,atomic_read(&obj->ps_thd_val_l));
           ...
           if(!test_bit(CMC_BIT_PS, &obj->enable)){
			...
			alsps_calibrate.x = obj->hw->ps_threshold_high + alsps_calibrate.z; // ps threshold high
			alsps_calibrate.y = obj->hw->ps_threshold_low + alsps_calibrate.z; // ps threshold low
...
//  *value = temp_value;
    ...
    if(ir_value < 200)    //cwf and office light
    {
        ...
        *value = (obj->als)*1123/1000;  //kevindang20150918
...
//      printk("---CW and Office---als value lux = %d\n",obj->als);
    ...
    else   //D65 and A light
    {
        ...
        *value = (obj->als)*1000/1450;     //Sam  20150713
        *status = SENSOR_STATUS_ACCURACY_MEDIUM;
