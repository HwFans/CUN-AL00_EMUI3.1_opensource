...
/*----------------------------------------------------------------------------*/
	...
	#define STK_LT_N_CT	20
	#define STK_HT_N_CT	50
...
#ifdef CALI_EVERY_TIME
	obj->ps_high_thd_boot = obj->hw->ps_high_thd_val;
	obj->ps_low_thd_boot = obj->hw->ps_low_thd_val;
...
static ssize_t stk3x1x_store_config(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	{
		APS_ERR("invalid content: '%s', length = \n", buf);
	}
	return count;    
}
/*----------------------------------------------------------------------------*/
...
static int als_enable_nodata(int en)
{
    ...
    if(en)
	{
		if((res = stk3x1x_enable_als(stk3x1x_obj->client, 1)))
		{
			APS_ERR("enable als fail: %d\n", res); 
			return -1;
		}
	...
	{
		if((res = stk3x1x_enable_als(stk3x1x_obj->client, 0)))
		{
			APS_ERR("disable als fail: %d\n", res); 
			return -1;
		}
...
static int als_get_data(int* value, int* status)
{
	...
	{
		*value = stk3x1x_get_als_value(stk3x1x_obj, stk3x1x_obj->als);
...
static int ps_enable_nodata(int en)
{
    ...
    if(en)
	{
		if((res = stk3x1x_enable_ps(stk3x1x_obj->client, 1, 1)))
		{
			APS_ERR("enable ps fail: %d\n", res); 
			return -1;
		}
	...
	{
		if((res = stk3x1x_enable_ps(stk3x1x_obj->client, 0, 1)))
		{
			APS_ERR("disable ps fail: %d\n", res); 
			return -1;
		}
