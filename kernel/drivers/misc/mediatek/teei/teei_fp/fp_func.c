...
//add by huaqin zam 20150917 start
extern void fpc_spi_enable_clk(void);
extern void fpc_spi_disable_clk(void);
//add by huaqin zam 20150917 end
...
static const struct file_operations fp_fops = {.owner = THIS_MODULE, .llseek =
		fp_llseek, .read = fp_read, .write = fp_write, .unlocked_ioctl =
		fp_ioctl, .compat_ioctl = fp_ioctl, .open = fp_open, .release =
		fp_release, };
