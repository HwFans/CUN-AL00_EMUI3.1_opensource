...
static void fiq_drivers_init(struct service_handler *handler) /*! 服务初始化处理 */
{
	register_shared_param_buf(handler);
}

static void fiq_drivers_deinit(struct service_handler *handler) /*! 服务停止处理 */
{
	return;
}

static int  fiq_drivers_handle(struct service_handler *handler) /*! 服务调用 */
{
	return 0;
}
/******************************TIME**************************************/
...
static void reetime_init(struct service_handler *handler) /*! 服务初始化处理 */
{
	register_shared_param_buf(handler);

}

static void reetime_deinit(struct service_handler *handler) /*! 服务停止处理 */
{
	return;
}

static int  __reetime_handle(struct service_handler *handler) /*! 服务调用 */
{
	struct timeval tv;
	void *ptr = NULL;
	int tv_sec;
...
static int reetime_handle(struct service_handler *handler) 
{
	...
	/* with a wmb() */
	wmb();
	smp_call_function_single(0, secondary_reetime_handle, (void *)(&reetime_handle_entry), 1);
...
static void socket_init(struct service_handler *handler) /*! 服务初始化处理 */
{
...
static void socket_deinit(struct service_handler *handler) /*! 服务停止处理 */
{
	kfree((void *)para_vaddr);
	kfree((void *)buff_vaddr);
}

static int  socket_handle(struct service_handler *handler) /*! 服务调用 */
{
...
static void vfs_init(struct service_handler *handler) /*! 服务初始化处理 */
{
	register_shared_param_buf(handler);
...
static void vfs_deinit(struct service_handler *handler) /*! 服务停止处理 */
{
	return;
}


static int __vfs_handle(struct service_handler *handler) /*! 服务调用 */
{
...
static int vfs_handle(struct service_handler *handler)   
{
	...
	down(&smc_lock);
        ...
        /* with a wmb() */
        wmb();
        smp_call_function_single(0, secondary_vfs_handle, (void *)(&vfs_handle_entry), 1);
...
static void printer_driver_init(struct service_handler *handler) /*! 服务初始化处理 */
{
...
static void printer_driver_deinit(struct service_handler *handler) /*! 服务停止处理 */
{
	return;
}

static int printer_driver_handle(struct service_handler *handler) /*! 服务调用 */
{
...
{
	smp_call_function_single(0, secondary_invoke_fastcall, NULL, 1);
}
			...
			unsigned long bdrv_phy_address, unsigned long tlog_phy_address)
{
	...
	smp_call_function_single(0, secondary_init_cmdbuf, (void *)(&init_cmdbuf_entry), 1);
