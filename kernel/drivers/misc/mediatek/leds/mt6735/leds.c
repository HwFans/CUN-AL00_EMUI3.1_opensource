...
extern  S32 set_huawei_torch_duty(U16 duty);
/*****************PWM *************************************************/
...
int mt_led_blink_pmic(enum mt65xx_led_pmic pmic_type, struct nled_setting *led)
{
	...
	switch(pmic_type){
		case MT65XX_LED_PMIC_NLED_ISINK0:
			...
			pmic_set_register_value(PMIC_RG_ISINK0_DOUBLE_EN, 1);
		...
		case MT65XX_LED_PMIC_NLED_ISINK1:
			...
			pmic_set_register_value(PMIC_RG_ISINK1_DOUBLE_EN, 1);
...
/*
sub flash on torch this is on isink_0, double is en, cur is 40MA  
 add by allen wang 2015 11 25
 */
int mt_brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	static bool first_time = true;
	static int isink_cur = 0;
	static int double_mask = 0;
	...
	mutex_lock(&leds_pmic_mutex);
		...
		else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK_SUBFLASH)
		{
			...
			double_mask = 1;	
			isink_cur = ((level - 16)/16);
			}else{
			double_mask = 0;
			isink_cur = 0;
			level = 0;
			}
			LEDS_DEBUG(" [LED]PMIC#%d:%d wangyang  double_mask = %d, isink_cur = %d, \n", pmic_type, level,double_mask,isink_cur);
			//isink0
			pmic_set_register_value(PMIC_RG_DRV_32K_CK_PDN,0x0); // Disable power down  
			pmic_set_register_value(PMIC_RG_DRV_ISINK0_CK_PDN,0);
			pmic_set_register_value(PMIC_RG_DRV_ISINK0_CK_CKSEL,0);
			pmic_set_register_value(PMIC_ISINK_CH0_MODE,ISINK_REGISTER_MODE);
			pmic_set_register_value(PMIC_ISINK_CH0_STEP,isink_cur);//4mA
			pmic_set_register_value(PMIC_RG_ISINK0_DOUBLE_EN, double_mask);
			pmic_set_register_value(PMIC_ISINK_DIM0_DUTY,15);
			pmic_set_register_value(PMIC_ISINK_DIM0_FSEL,ISINK_1KHZ);//1KHz
			//isink1
			pmic_set_register_value(PMIC_RG_DRV_32K_CK_PDN,0x0); // Disable power down  
			...
			pmic_set_register_value(PMIC_ISINK_CH1_STEP,isink_cur);//24mA
			pmic_set_register_value(PMIC_RG_ISINK1_DOUBLE_EN, double_mask);
			pmic_set_register_value(PMIC_ISINK_DIM1_DUTY,15);
			pmic_set_register_value(PMIC_ISINK_DIM1_FSEL,ISINK_1KHZ);//1KHz
			if (level) 
			{
	...
	*/	
	...
	switch (cust->mode) {
		...
		case MT65XX_LED_MODE_PMIC:
			...
			if((strcmp(cust->name,"sub-flashlight") == 0)) {
				switch (cust->data) {
					case MT65XX_LED_PMIC_NLED_ISINK0_1_PARALLEL:
						button_flag_isink0 = 1;
						//button_flag_isink1 = 1;
						return mt_brightness_set_pmic(MT65XX_LED_PMIC_NLED_ISINK0, level, bl_div_hal);
						//return mt_brightness_set_pmic(MT65XX_LED_PMIC_NLED_ISINK1, level, bl_div_hal);
				}
			}
		if((strcmp(cust->name,"hq-torch") == 0)) {
				printk("wangyang the torch level  = %d \n",level);
				//switch (cust->data) {
				//	case MT65XX_LED_PMIC_NLED_TORCH:
						//return set_huawei_torch_duty(level);
				//}
			}		
